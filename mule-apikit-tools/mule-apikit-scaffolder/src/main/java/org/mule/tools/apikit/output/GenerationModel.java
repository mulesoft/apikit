...
public class GenerationModel implements Comparable<GenerationModel> {
  ...
  public GenerationModel(
                         API api, String version, IResource resource, IAction action, String mimeType) {
    this.api = api;
    ...
    this.action = action;
    this.splitPath =
        new ArrayList<>(Arrays.asList(this.resource.getResolvedUri(version).split("/")));
  ...
  private String getExampleWrapper(Map<String, IResponse> responses) {
    ...
    final LinkedHashMap<String, IResponse> validResponses =
        responses.entrySet().stream()
            //        .filter(entry -> isOkResponse(entry.getKey()))
            .sorted(getStatusCodeComparator())
            .collect(
                     toMap(
                           (Map.Entry<String, IResponse> e) -> OAS_DEFAULT_STATUS_CODE.equalsIgnoreCase(e.getKey()) ? "200"
                               : e.getKey(),
                           Map.Entry::getValue,
                           (k, v) -> v,
                           LinkedHashMap::new));
  ...
  private static Comparator<Map.Entry<String, IResponse>> getStatusCodeComparator() {
    return (c1, c2) -> {
      ...
      if (OAS_DEFAULT_STATUS_CODE.equalsIgnoreCase(c1Key)
          && OAS_DEFAULT_STATUS_CODE.equalsIgnoreCase(c2Key))
        return 0;
    ...
    if (action.getResponses() != null) {
      for (String response : action.getResponses().keySet()) {
        ...
        if (statusCode >= 200 && statusCode < 299) {
          if (action.getResponses().get(response).getBody() != null
              && action.getResponses().get(response).getBody().size() > 0) {
  ...
  public String getFlowName() {
    ...
    flowName
        .append(action.getType().toString().toLowerCase())
        .append(FLOW_NAME_SEPARATOR)
        .append(resource.getResolvedUri(version));

    if (mimeType != null) {
      flowName.append(FLOW_NAME_SEPARATOR).append(mimeType);
    }

    if (api.getConfig() != null && !StringUtils.isEmpty(api.getConfig().getName())) {
      flowName.append(FLOW_NAME_SEPARATOR).append(api.getConfig().getName());
    }
