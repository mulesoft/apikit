 ...
 * MIME-Type Parser
 *
 * <p>This class provides basic functions for handling mime-types. It can handle matching mime-types
...
public class MimeTypeParser {
  ...
  protected static FitnessAndQuality fitnessAndQualityParsed(
                                                             MediaType target, List<MediaType> parsedRanges) {
    int bestFitness = -1;
    float bestFitQ = 0;
    ...
    for (MediaType range : parsedRanges) {
      if ((target.type().equals(range.type())
        ...
        for (String k : target.parameters().keySet()) {
          int paramMatches = 0;
          if (!k.equals("q")
              && range.parameters().containsKey(k)
      ...
      if ((target.type.equals(range.type) || range.type.equals("*") || target.type.equals("*"))
        ...
        for (String k : target.params.keySet()) {
          ...
          if (fitness > bestFitness) {
            ...
            if (range.type.equals("*") && range.subType.equals("*")) {
              bestFitQ = NumberUtils.toFloat(target.params.get("q"), 0);
            } else {
   ...
   * media-ranges listed in header. The value of header must be a string that conforms to the format
   * of the HTTP Accept: header. The value of 'supportedRepresentations' is a list of mime-types.
   *
   * <p>MimeParse.bestMatch(Arrays.asList(new String[]{"application/xbel+xml", "text/xml"}),
  ...
  public static MediaType bestMatch(List<String> supportedRepresentations, String header) {
    ...
    for (String representation : supportedRepresentations) {
      FitnessAndQuality fitnessAndQuality =
          fitnessAndQualityParsed(representation + ";q=" + quality, parseResults);
  ...
  public static boolean isMediaTypeAcceptable(
                                              List<MediaType> acceptContentTypes, MediaType mediaType) {
