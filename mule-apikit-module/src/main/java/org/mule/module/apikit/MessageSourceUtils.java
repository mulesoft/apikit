...
public class MessageSourceUtils {
  ...
  /**
   * Extracts the configured HTTP URI from a flow. It only works for flows that uses the HTTP
   * extension.
   *
   * <p>// * @param flow where to extract the URI
   *
   * @return the URI
   */
  public static URI getUriFromFlow(Component source) {
    if (source != null && isHttpExtensionSource(source)) {
      try {
        ...
        LOGGER.warn(
                    "Error getting uri from flow " + source.getLocation().getRootContainerName(), e);
      }
    }
  ...
  private static boolean isHttpExtensionSource(Component source) {
    final ComponentIdentifier identifier =
        source.getLocation().getComponentIdentifier().getIdentifier();
