...
public class FlowUtils {
  ...
  public static List<Flow> getFlowsList(ConfigurationComponentLocator locator) {
    return cast(
                locator.find(
                             ComponentIdentifier.builder().name(FLOW_TAG_NAME).namespace(MULE_NAMESPACE).build()));
  }

  public static Optional<Component> getSource(
                                              ConfigurationComponentLocator locator, String flowName) {
    return locator.find(Location.builder().globalName(flowName).addSourcePart().build());
  }

  public static Optional<URI> getSourceLocation(
                                                ConfigurationComponentLocator locator, String flowName) {
