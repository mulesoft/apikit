...
public class UrlencodedFormV2Validator implements FormValidatorStrategy<TypedValue> {
  ...
  private void validateAndAddDefaults(MultiMap<String, String> requestMap)
      ...
      if (values.isEmpty()) {
        ...
        if (defaultValue != null) {
          ...
          throw new InvalidFormParameterException(
                                                  "Required parameter " + expectedKey + " not specified");
        }
      } else {
        if (parameter.isRepeat() || parameter.isArray()) {
          ...
          throw new InvalidFormParameterException(
                                                  "Parameter '" + expectedKey + "' is not repeatable");
        else {
  ...
  private void validate(String expectedKey, IParameter parameter, String value)
      throws InvalidFormParameterException {
    if (!parameter.validate(value))
      throw new InvalidFormParameterException(
                                              "Invalid value '" + value + "' for parameter " + expectedKey);
  }
  ...
  private void validateAsArray(String expectedKey, IParameter parameter, List<String> values)
    ...
    if (!parameter.validate(valueToValidate)) {
      ...
      if (!parameter.validate(values.stream().map(v -> "- '" + v + "'").collect(joining("\n")))) {
        throw new InvalidFormParameterException(
                                                "Invalid value '" + valueToValidate + "' for parameter " + expectedKey);
      }
    }
  }
}
