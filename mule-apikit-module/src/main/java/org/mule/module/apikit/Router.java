...
public class Router extends AbstractComponent implements Processor, Initialisable, AbstractRouter {
  ...
  public void initialise() throws InitialisationException {
    ...
    if (!url.isPresent()) {
      LOGGER.error(
                   "There was an error retrieving api source. Console will work only if the keepApiBaseUri property is set to true.");
      return;
    }
    ...
    LOGGER.info(
                StringMessageUtils.getBoilerPlate(
                                                  "APIKit Router for config '"
                                                      + configurationName
                                                      + "' started using Parser: "
  ...
  private Publisher<CoreEvent> doRoute(
                                       CoreEvent event,
    ...
    String path =
        UrlUtils.getRelativePath(attributes.getListenerPath(), attributes.getRawRequestPath());
    ...
    eventBuilder =
        validateRequest(event, eventBuilder, config, resource, attributes, resolvedVariables);
    String contentType = AttributesHelper.getMediaType(attributes);
    Flow flow =
        config.getFlowFinder().getFlow(resource, attributes.getMethod().toLowerCase(), contentType);

    final Publisher<CoreEvent> flowResult =
        processWithChildContext(
                                eventBuilder.build(), flow, ofNullable(getLocation()), flow.getExceptionListener());
                 ...
                 final String successStatusCode =
                     config
                         ...
                         .getSuccessStatusCode(
                                               resource.getAction(attributes.getMethod().toLowerCase()));
                 return builder(result)
                     .addVariable(config.getHttpStatusVarName(), successStatusCode)
                     .build();
               }
               return result;
             });
  ...
  public CoreEvent.Builder validateRequest(
                                           CoreEvent event,
                                           CoreEvent.Builder eventBuilder,
