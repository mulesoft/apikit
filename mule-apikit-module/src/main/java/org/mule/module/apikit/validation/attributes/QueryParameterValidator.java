...
public class QueryParameterValidator {
      ...
      if (actual.isEmpty()) {
        if (expected.isRequired()) {
          throw new InvalidQueryParameterException(
                                                   "Required query parameter " + expectedKey + " not specified");
        }

        if (expected.getDefaultValue() != null) {
          queryString =
              AttributesHelper.addQueryString(
                                              queryString, queryParams.size(), expectedKey, expected.getDefaultValue());

          queryParams =
              AttributesHelper.addParam(queryParams, expectedKey, expected.getDefaultValue());
        }
      } else {

        if (actual.size() > 1 && !(expected.isRepeat() || expected.isArray())) {
          throw new InvalidQueryParameterException(
                                                   "Query parameter " + expectedKey + " is not repeatable");
        }
  ...
  private void validateQueryParametersStrictly(MultiMap<String, String> queryParams)
      throws InvalidQueryParameterException {
    // check that query parameters are defined in the RAML
    final Set notDefinedQueryParameters =
        difference(queryParams.keySet(), action.getQueryParameters().keySet());
    if (!notDefinedQueryParameters.isEmpty())
      throw new InvalidQueryParameterException(
                                               Joiner.on(", ").join(notDefinedQueryParameters)
  ...
  // only for raml 1.0
  private void validateQueryParamArray(
                                       String paramKey, IParameter expected, Collection<?> paramValues)
      throws InvalidQueryParameterException {
  ...
  private void validateQueryParam(String paramKey, IParameter expected, String paramValue)
      throws InvalidQueryParameterException {
    if (!expected.validate(paramValue)) {
      String msg =
          String.format(
                        "Invalid value '%s' for query parameter %s. %s",
