...
class DocumentParser {
  ...
  public static Document parseFile(final Parser parser, final URI uri, final boolean validate) throws ParserException {
    ...
    if (validate) {
      ...
      if (!parsingReport.conforms()) {
        ...
        if (!results.isEmpty()) {
          throw new ParserException(buildErrorMessage(uri, results));
        }
      }
    }
    return document;
  ...
  private static String buildErrorMessage(final URI uri, List<ValidationResult> validationResults) {
    final StringBuilder message = new StringBuilder(String.format("Invalid API '%s'. Errors: ", uri.toString()));
