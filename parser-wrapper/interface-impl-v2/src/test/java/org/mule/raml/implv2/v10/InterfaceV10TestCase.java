...
public class InterfaceV10TestCase {
  ...
  public void check() {
    ...
    assertThat(
               raml.getSchemas().get(0).get("User"),
               is(
                  "{\"$ref\":\"#/definitions/User\",\"definitions\":{\"User\":{\"type\":\"object\",\"properties\":{\"firstname\":{\"type\":\"string\"},\"lastname\":{\"type\":\"string\"},\"age\":{\"type\":\"number\"}},\"required\":[\"firstname\",\"lastname\",\"age\"]}},\"$schema\":\"http://json-schema.org/draft-04/schema#\"}"));
  ...
  public void references() {
    ...
    final String pathAsUri =
        requireNonNull(getClass().getClassLoader().getResource(relativePath)).toString();
    ...
    final List<String> paths =
        Arrays.asList(relativePath, pathAsUri, absoulutPath, pathAsRemoteUrl);
  ...
  private void checkReferences(String path, ResourceLoader resourceLoader) {
    ...
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r, "org/mule/raml/implv2/v10/references/address.raml", resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r,
                                                   "org/mule/raml/implv2/v10/references/company-example.json",
                                                   resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r, "org/mule/raml/implv2/v10/references/partner.raml", resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r, "org/mule/raml/implv2/v10/references/data-type.raml", resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r, "org/mule/raml/implv2/v10/references/library.raml", resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r, "org/mule/raml/implv2/v10/references/company.raml", resourceLoader)),
               is(true));
  }
    ...
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r,
                                                   "org/mule/raml/implv2/v10/exchange/exchange_modules/library1.raml",
                                                   resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r,
                                                   "org/mule/raml/implv2/v10/exchange/exchange_modules/library2.raml",
                                                   resourceLoader)),
               is(true));
    assertThat(
               allReferences.stream()
                   .anyMatch(
                             r -> endWithAndExists(
                                                   r,
                                                   "org/mule/raml/implv2/v10/exchange/exchange_modules/library3.raml",
                                                   resourceLoader)),
               is(true));
  }

  private boolean endWithAndExists(
                                   String reference, String goldenFile, ResourceLoader resourceLoader) {
